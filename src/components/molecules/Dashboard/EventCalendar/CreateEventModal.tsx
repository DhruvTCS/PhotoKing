...
import {
    ...
    deleteEventAPI,
    updateEventAPI,
} from '../../../../Redux/ApiCalls/Dashboard/EventAPI'
...
const EventCreateModal: React.FC<ModalProps> = ({
    isOpen,
    onClose,
    onSubmit,
    selectedSlot,
    ...
    useEffect(() => {
        if (currentEvent) {
            ...
            const formatedStart = `${start.getFullYear()}-${padZero(
                start.getMonth() + 1,
            )}-${padZero(start.getDate())}T${padZero(start.getHours())}:${padZero(
                start.getMinutes(),
            )}`
            const formatedEnd = `${end.getFullYear()}-${padZero(
                end.getMonth() + 1,
            )}-${padZero(end.getDate())}T${padZero(end.getHours())}:${padZero(
                end.getMinutes(),
            )}`
            // // console.log(start);
            setIsUpdate(true)
    ...
    const handleMember = (id: number) => {
        if (deleteMember.includes(id)) {
            ...
            if (selectedMembers.includes(id)) {
                ...
                if (flag) showErrorToast("Please remove first from sub event.")
                else
    ...
    const handleSubmit = () => {
        if (!validEventName(eventName) && subEvents.length < 0) {
            ...
            if (isUpdate && currentEvent) {
                ...
                dispatch(
                    updateEventAPI({
                        business_event_id: currentEvent.id,
                        title: eventName,
            ...
            // dispatch(updateEventAPI({ event_id: currentEvent.id, start_date: startDate, end_date: endDate, start_time: startTime, end_time: endTime, title: eventName, location: eventLocation, member_ids: [...selectedMembers, ...eventMembers] }))
                ...
                dispatch(
                    createEventAPI({
                        title: eventName,
                        event_member_id: selectedMembers,
                        sub_events: subEventsData,
                    }),
                )
            }
    ...
    const handleDeleteMember = (id: number) => {
        ...
        if (flag) showErrorToast("Please remove first from sub event.")
        else if (!deleteMember.includes(id) && eventMembers.includes(id)) {
    ...
    const handleAddSubEvents = (subEvent: CalendarSubEvents) => {
        ...
        const notIncludedmembers = subEvent.members.filter(
            (e) =>
                !selectedMembers.includes(e.member) && !eventMembers.includes(e.member),
        )
        setSelectedMembers((pre) => [
            ...pre,
            ...notIncludedmembers.map((member) => member.id),
        ])
    ...
    return selectedSlot || currentEvent ? (
        <ModalOverlay>
            <ModalContent>
                ...
                <ModalTitleConatiner>
                    {currentEvent ? (
                        <>
                            {deletePopup && (
                                <DeleteEventPopup
                                    Delete={() => handleDeleteEvent(currentEvent.id)}
                ...
                <InputNameConatiner>
                    ...
                    <ModalInput
                        ...
                        value={eventName}
                        onChange={(e) => {
                            if (e.target.value.length < 35) setEventName(e.target.value)
                        }}
                                            ...
                                            selectedMembers.includes(parseInt(member.id)) && (
                                                <SelectedMemberDataConatiner>
                                                    <SelectedMemberData>
                                                        ...
                                                        <CloseButton
                                                            onClick={() => handleMember(parseInt(member.id))}
                                                        >
                                                            &times;
                                                        </CloseButton>
                                                    </SelectedMemberData>
                ...
                <SubmitConatiner>
                    <SubmitButton
                        text={currentEvent ? 'Update' : 'Create'}
                        width={270}
                        needArrow={false}
